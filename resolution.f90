...
program resolution_test
implicit none
integer::j,n,k,np
real*8 ::h,pi,xp,fp
real*8,allocatable::x(:),u(:),upe(:)
real*8,allocatable::up1(:),up2(:),up3(:),up4(:),up5(:)
...
h = 1.0d0/dfloat(n)
	...
	x(j)   = dfloat(j)*h
	!function
	u(j)   = dsin(2.0d0*pi*dfloat(k)*x(j))
	!exact solution for derivative
	upe(j) = 2.0d0*pi*dfloat(k)*dcos(2.0d0*pi*dfloat(k)*x(j))
	end do

	!numerical solutions for the derivative 
	allocate(up1(0:n),up2(0:n),up3(0:n),up4(0:n),up5(0:n))
...
write(12,*)'variables ="x","upe"'
	do j=0,np
		xp = dfloat(j)*1.0d0/dfloat(np)
		fp = 2.0d0*pi*dfloat(k)*dcos(2.0d0*pi*dfloat(k)*xp)
		write(12,*) xp,fp
	end do
close(12)
...
!-----------------------------------------------------------------------------!
!c6: 	6th-order central difference scheme for the first order derivative(up)	
!-----------------------------------------------------------------------------!
subroutine c6(u,up,h,n)
implicit none
integer :: n,i
real*8  :: h
real*8, dimension (0:n)	:: u,up
...
do i=3,n-3
up(i) = (u(i+3)-9.0d0*u(i+2)+45.0d0*u(i+1)-45.0d0*u(i-1)+9.0d0*u(i-2)-u(i-3))/(60.0d0*h)
end do

!fourth-order close to boundary
i=2
up(i) = (-u(i+2)+8.0d0*u(i+1)-8.0d0*u(i-1)+u(i-2))/(12.0d0*h)
